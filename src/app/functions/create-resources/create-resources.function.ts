...
export async function createResources<T>(params: CreateResourcesParams<T>): Promise<boolean> {
	params.resources.update((resources) =>
		resources.map((resource) => (resource.status === "created" ? resource : { ...resource, status: "loading" })),
	);
	const resultPromises = params
		.resources()
		.filter((resource) => resource.status === "loading")
		.map((resource) =>
			params
				.createFn(resource.model)
				...
				.then((result) => {
					params.resources.update((resources) =>
						resources.map((r) => (params.equalFn(r.model, result.model) ? result : r)),
					);
